...
template<class T> inline void RC(T &x){
    //cin >> x;
    scanf(" %c", &x);
...
struct Node{
    ...
    Node(int _c):l(NULL), r(NULL), p(NULL), c(_c){}
...
void Insert(int h, int p, int c){
    ...
    Node *x = new Node(c); TW.insert(MP(p, x));
    set<PIN>::iterator _it = TW.lower_bound(MP(p, x)), it;

    if (c == WHITE) it = _it, ++it; else{
        it = TW.lower_bound(MP(p + L, (Node*) NULL));
        B[h] = x, TB.insert(MP(p, h));
    }
    ...
    if (_it != TW.begin()){
        it = _it, --it;
        if (y->c == WHITE) y->link(x);
    }
}

void Insert(int h){
    Insert(h, A[h] + L, WHITE), Insert(h, A[h], BLACK);
}

inline void Black_to_White(int h, int p, Node *x){
    x->c = WHITE; set<PIN>::iterator it=TW.upper_bound(MP(p, x));
    B[h]->link(it==TW.end() ? NULL : it->second);
    TB.erase(make_pair(p, h));
}
...
int main(){
    ...
    //freopen("Poison09.out", "w", stdout);
#endif

    RD(n, L, m), ++L;

    REP(i, n) RD(A[i]), Insert(i);

    int h, p; DO(m){
        RD(h, p), Black_to_White(h, A[h], B[h]);
        A[h] = p, Insert(h);
        OT(B[TB.begin()->second]->access()->s);
    }
}
