...
int Gao()
{
    ...
    int sta,now,sta0,sta1,sta2,endsta,step;
    endsta = 0;
    for (int i = 1; i <= 4; i++)
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 6; j++)
            if (mp[i][j] == 'G')
                sta0 = setbit(1,sta0,i,j);
    sta1 = 0;
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 6; j++)
            if (mp[i][j] == 'B')
                sta1 = setbit(1,sta1,i,j);
    sta2 = 0;
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 6; j++)
            if (mp[i][j] == 'W')
                sta2 = setbit(1,sta2,i,j);
    Q.push(make_pair(endsta,0));
    inq[endsta] = true;
    while (!Q.empty())
    {
        now = Q.front().first;
        step = Q.front().second;
        Q.pop();
        if (now == sta0)  return step;
        if (now == sta1)  return step;
        if (now == sta2)  return step;
        for (int i = 0; i < 4; i++)
        {
            sta = shiftrow0(now,i);
            if (inq[sta] == false)
