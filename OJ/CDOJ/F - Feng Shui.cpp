...
int n,resn;
Point p[100],res[100],c[2];
Line line[100];
double r,polar,mindis;
...
int main()
{
    while (scanf("%d%lf",&n,&r) != EOF)
    {
        ...
        for (int i = 0;i < n;i++)
        {
            ...
            polar += pi/2.0;
            line[i] = Line(Point(p[i].x+r*cos(polar),p[i].y+r*sin(polar)),
                        Point(p[(i+1)%n].x+r*cos(polar),p[(i+1)%n].y+r*sin(polar)));
        }
        HalfPlaneIntersection(line,n,res,resn);
        mindis = 0.0;
        for (int i = 0;i < resn;i++)
            for (int j = 0;j < resn;j++)
                if (Point(res[i],res[j]).Length() >= mindis)
                {
                    mindis = Point(res[i],res[j]).Length();
                    c[0] = res[i];
                    c[1] = res[j];
                }
