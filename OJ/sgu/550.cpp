...
struct SplayTree
{
    ...
    void Pushdown(Node *x)
    {
        ...
        for (int i = 0; i < 2; i++)
            if (x->c[i] != nil)
            {
                printf("node %d, c = %d, la = %d, lb = %d\n",x-mem,x->c[i]-mem,x->la,x->lb);
    ...
    Node *Access(Node *u)
    {
        ...
        while (u != nil)
        {
			printf("Access : u = %d, v = %d, u->la = %d, u->lb = %d\n",u-mem,v-mem,u->la,u->lb);
            Splay(u);
			printf("u->la = %d, u->lb = %d\n",u->la,u->lb);
            v->p = u;
            u->c[1] = v;
            Pushup(u);
            u = (v = u)->p;
...
int main()
{
    ...
    for (int i = 0; i < n-1; i++)
    {
        ...
        sp.Access(u);
		printf("u.a = %d, u.b = %d, u.la = %d, u.lb = %d\n",u->a,u->b,u->la,u->lb);
        ...
        u->la = (long long)u->la*wi%mod;
        u->lb = (long long)u->lb*wi%mod;
