...
bool inside(int xi,int yi,int ri,int xj,int yj,int rj)
{
    return sqrt((xi-xj)*(xi-xj)+(yi-yj)*(yi-yj))+ri < rj;
}
...
string CirclesGame::whoCanWin(vector <int> x, vector <int> y, vector <int> r)
{
    ...
    for (int i = 0; i < n; i++)
    {
        fa[i] = n;
        for (int j = 0; j < n; j++)
            if (i != j && inside(x[i],y[i],r[i],x[j],y[j],r[j]))
            {
                if (fa[i] == n)
                    fa[i] = j;
                else if (inside(x[j],y[j],r[j],x[fa[i]],y[fa[i]],r[fa[i]]))
                    fa[i] = j;
            }
    }
    for (int i = 0; i < n; i++)
