var n,m,i,min,ta,tb,na,nb,now,temp:longint;
...
min:=5000;
qsort(1,m);
now:=1; ta:=l[now]; tb:=r[now] ;  temp:=0;
repeat
inc(now); na:=l[now]; nb:=r[now];
if ( na >= ta ) and ( na <= tb ) then
 begin
 if nb <= tb then
 begin
 if b[na] then begin b[na]:=false;  inc(temp); end;
 if b[nb] then begin b[nb]:=false;  inc(temp); end;
 end
 else
 begin
 if b[na] then begin b[na]:=false;  inc(temp); end;
 if b[ta] then begin b[tb]:=false;  inc(temp); end;
 nb:=tb;
 end;
 end
else
