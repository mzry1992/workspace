...
void convexHull(Point p[],int n,int res[],int& resn,int hullType) // 1 del the point on edge , 0 does not
{
    ...
    for (int i = 2;i < n;i++)
    {
        while (resn > 1 && xmult(Point(p[res[resn-1]],p[res[resn-2]]),Point(p[res[resn-1]],p[i]))+(long long)hullType > 0)
            resn--;
        res[resn] = i;
        resn++;
    }
    while (resn > 1 && xmult(Point(p[res[resn-1]],p[res[resn-2]]),Point(p[res[resn-1]],p[res[0]]))+hullType > 0)
        resn--;
    if (resn < 3)   resn = 0;
}
