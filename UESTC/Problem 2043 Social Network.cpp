...
void ShowHomePage(int id,char myname[])
{
    ...
    {
        tid = member[id].fri[i];
        for (int j = 0; j < member[tid].rec.size(); j++)
        {
            recid = member[tid].rec[j];
            if (recid <= member[id].lastread)   continue;
            if (recoder[recid].typ == 6)
            {
                int id1 = name2id[recoder[recid].sa];
                int id2 = name2id[recoder[recid].sb];
                if (id1 == id || id2 == id)
                    rec[totrec++] = recid;
            }
            else
    ...
    //if (totrec-1 >= 0)
    //    member[id].lastread = rec[totrec-1];
}
...
void Says()
{
    if (name2id.find(nowOper.sa) != name2id.end())
    {
        ...
        member[id].rec.push_back(totrecoder);
        recodertime[totrecoder] = nowTime;
        recoder[totrecoder++] = nowOper;
        outTime(nowTime);
        printf(" %s said : %s\n",nowOper.sa,nowOper.sb);
